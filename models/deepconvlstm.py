...
from tensorflow.keras.layers import Input, Layer, Dense, Flatten, Activation, Permute, Multiply, Dot, Concatenate, RepeatVector, LSTM, Conv2D, Lambda
...
def DeepConvLSTM_Encoder(input_shape, LSTM_units=32, num_conv_filters=3, F=32, D=10):
    cnn_inputs = Input(shape=(input_shape[0], input_shape[1], 1))
    cnn_output = Conv2D(num_conv_filters, kernel_size = (1, input_shape[1]), strides=(1, 1), padding='valid', data_format="channels_last")(cnn_inputs)

    sq_layer_out = Lambda(lambda x: tf.keras.backend.squeeze(x, axis = 2))(cnn_output)
    rnn_layer_output, _, _  = LSTM(LSTM_units, return_sequences=True, return_state=True)(sq_layer_out)

    encoder_output, attention_weights = SelfAttention(size=F, num_hops=D, use_penalization=False)(rnn_layer_output)
    # dense_layer = Dense(num_labels, activation = 'softmax', name="classified")
    # dense_layer_output = dense_layer(encoder_output)

    model = Model(inputs=cnn_inputs, outputs=encoder_output)
    return model